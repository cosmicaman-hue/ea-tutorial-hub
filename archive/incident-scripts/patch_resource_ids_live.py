...
in the LIVE database to fix studentId mismatches.

Uses C:/var/data/ea_tutorial_hub/offline_scoreboard_data.json (Flask live path).
"""
...
def load():
    return json.load(open(LIVE_PATH, 'r', encoding='utf-8'))

def save(d):
    backup = LIVE_PATH.replace('.json', f'.bak-resource-id-fix-{datetime.now().strftime("%Y%m%d_%H%M%S")}.json')
    shutil.copy2(LIVE_PATH, backup)
    print(f"Backup: {backup}")
    with open(LIVE_PATH, 'w', encoding='utf-8') as f:
        json.dump(d, f, ensure_ascii=False, separators=(',', ':'))
    print("Saved to LIVE database.")
...
def main():
    ...
    print("=" * 80)
    print(f"RESOURCE ID PATCH PLAN (LIVE DATABASE) — {len(all_patches)} rows to fix")
    ...
    if mode == 'patch':
        print("\nApplying patches to LIVE database...")
        patched = 0
        for table in ['resource_transactions', 'resource_requests', 'resource_advantage_deductions']:
            rows = d.get(table, [])
            for row in rows:
        ...
        print("\nRun with 'patch' argument to apply fixes to LIVE database.")
