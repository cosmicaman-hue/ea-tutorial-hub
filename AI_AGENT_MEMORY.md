...
| Stars/VETO | `getAvailableStarsForMonth` 49192, `validateStarAvailability` 49223, ~~`smartStarCounter`~~ (REMOVED 2026-07-25 — see §10.30), star transfer 20118, star→veto convert 20622, points→star 20765, veto workflow `promptVetoUsageWorkflow` 19609 / `applyVetoWorkflowDatabaseChanges` 19786, role veto quotas 18192–18461, `applyPostHolderVeto` 49370, **`stars_updated_at` ledger guard** (see §10.30) |
...
32. **2026-07-25 — stale-sync star balance overwrite + dead double-deduction landmine — FIXED/REMOVED**:
    - **Symptom investigated:** Student EA25H05 reported "should have 11 stars, shows 8". Diagnostic script `scripts/diag_star_ledger.py` confirmed the displayed `8` is **arithmetically correct** (11 June carry-in − 3 July usages on the 10th/12th/19th, each a single `-1` `star_usage_normal`). No active double-deduction was occurring.
    - **Root cause class (latent, not the trigger here):** `mergeStudentsSuperset` (fe ~L17911) and `_merge_students_preserve_active` (be `scoreboard.py` L3256) used timestamp winner-takes-all for the `stars` field. Sync ops bump `updated_at` artificially, so a stale snapshot with fewer stars but a newer `updated_at` could silently overwrite a correct higher balance — the documented root cause of past "should be N, shows M" reports (see `scripts/reconcile_stars.py` header and `scripts/anti_corruption_check.py` STAR STATE DRIFT detection).
    - **Fix — `stars_updated_at` ledger guard:** A new star-specific timestamp `student.stars_updated_at` is stamped on every star mutation and propagated through sync. Both merge functions now pick `stars` from the side with the newer `stars_updated_at` (falling back to `updated_at` for legacy records missing the field), so a sync-bumped `updated_at` on a stale snapshot can no longer lower a genuine star balance. Stamping sites: fe `db.addScore::applyStarVetoDelta` (~L12809), `db.deleteScore` (~L12878), star→veto convert (~L22696), `removeStudentStars` (~L28290), historical editor recompute (~L28990), admin repair `applyApprovedScoreAdjustment` (~L26473 / ~L41871), `updateStudentStarCounter` (~L53604); be `_upsert_score_delta` (`scoreboard.py` ~L3988) propagates the stamp to the student record on star deltas.
